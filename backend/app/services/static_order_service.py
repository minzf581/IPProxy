...
class StaticOrderService:
    ...
    async def create_order(
        ...
        """创建静态代理订单"""
        logger = logging.getLogger(__name__)
        logger.info(f"开始创建静态代理订单: user_id={user_id}, agent_id={agent_id}")
        ...
        try:
            ...
            logger.info(f"用户当前余额: {user.balance}")
            amount = order_data.get('total_cost', 0)
            if user.balance < amount:
                logger.error(f"用户余额不足: balance={user.balance}, amount={amount}")
            ...
            logger.info(f"支付处理结果: {payment_result}")
            ...
            try:
                ...
                logger.info(f"产品信息查询结果: {product_result}")

                if not product_result or product_result == "null":
                    logger.error(f"产品信息查询失败，产品编号: {order_data.get('product_no')}")
                    # 发起退款
                ...
                try:
                    ...
                    logger.error(f"解析产品信息失败: {product_result}")
                    await payment_service.refund_order(
                        user_id=user_id,
                        agent_id=agent_id,
                        order_no=order_no,
                ...
                if not product_found:
                    logger.error(f"产品不存在: {product_no}")
                    await payment_service.refund_order(
                        user_id=user_id,
                        agent_id=agent_id,
                        order_no=order_no,
                ...
                # 调用 IPIPV API 创建订单
                logger.info("开始调用 IPIPV API 创建订单")
                ...
                logger.info(f"IPIPV API 响应: {api_result}")
                
                if not api_result:
                    logger.error("IPIPV API 调用失败")
                    # 发起退款
                    await payment_service.refund_order(
                        user_id=user_id,
                        agent_id=agent_id,
    ...
    async def handle_callback(
        ...
        """
        try:
            logger.info(f"处理回调请求: type={type}, no={no}, op={op}")
            
            if type == 'order':
                order = self.db.query(StaticOrder).filter_by(app_order_no=no).first()
                if not order:
                    logger.error(f"订单不存在: {no}")
                ...
                order_info = await self.ipipv_api._make_request(
                    ...
                    {
                        'appOrderNo': no,
                        'orderNo': order.order_no  # 添加订单号参数
                    }
                )
                
                if not order_info:
                    logger.error(f"获取订单信息失败: {no}")
                ...
                try:
                    ...
                    logger.error(f"解析订单信息失败: {order_info}")
                    return {'code': 'failed', 'msg': '解析订单信息失败'}
                ...
                try:
                    self.db.commit()
                    logger.info(f"回调处理成功: order_no={order.order_no}, status={order.status}")
                except SQLAlchemyError as e:
                    self.db.rollback()
                ...
                try:
                    self.db.commit()
                    logger.info(f"实例状态更新成功: instance_no={no}, status={instance.status}")
                except SQLAlchemyError as e:
                    self.db.rollback()
            ...
            logger.error(f"处理回调请求时发生未知错误: {str(e)}")
            logger.exception(e)
