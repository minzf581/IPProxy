      ...
      <el-form :inline="true" :model="queryParams" class="form-inline">
        <el-form-item label="资源类型">
          <el-select v-model="queryParams.resourceType" placeholder="选择资源类型" clearable>
            <el-option label="动态资源" value="dynamic" />
            <el-option label="静态资源" value="static" />
          </el-select>
        </el-form-item>
    ...
    >
      ...
      <el-table-column label="资源类型" width="100">
        <template #default="{ row }">
          <el-tag :type="row.resourceType === 'dynamic' ? 'success' : 'warning'">
            {{ row.resourceType === 'dynamic' ? '动态' : '静态' }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column label="IP信息" width="200">
        <template #default="{ row }">
          <div>IP：{{ row.ip }}</div>
          <div>端口：{{ row.port }}</div>
          <div v-if="row.resourceType === 'static'">ISP：{{ row.isp }}</div>
        </template>
      </el-table-column>
      <el-table-column label="流量信息" width="200">
        <template #default="{ row }">
          <template v-if="row.resourceType === 'dynamic'">
...
const getResourceList = async () => {
  loading.value = true
  try {
    const { data } = await resourceStore.getAgentResources(queryParams)
...
const handleViewPassword = async (row) => {
  try {
    const { data } = await resourceStore[
      row.resourceType === 'dynamic' ? 'getDynamicPassword' : 'getStaticPassword'
    ](row.id)
...
const submitRenew = async () => {
  ...
  await renewFormRef.value.validate(async (valid) => {
    if (valid) {
      try {
        await resourceStore.renewResource(renewForm)
